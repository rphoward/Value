...
def prepare_gate_accept(
    ...
    *,
    ...
    """Gate UX: autofill decisions[] and hold gate_pending on pass.
...
def validation_milestone(session: dict[str, Any]) -> str:
    ...
    for module in reversed(MODULE_ORDER):
        if outcomes[module] in {"completed", "bypassed"}:
            label = MODULE_BRIEF_LABEL.get(module, module)
...
def active_phase(session: dict[str, Any]) -> str:
    ...
    last = MODULE_ORDER[-1]
    if position.get("module") == last and module_outcome(session, last) in {
        "completed",
        "bypassed",
    }:
        return "Complete"
    return MODULE_PHASE.get(position["module"], MODULE_PHASE.get(ENTRY_MODULE, "Start"))
...
def recompute_ledger(session: dict[str, Any]) -> dict[str, Any]:
    ...
    last = MODULE_ORDER[-1]
    if module_outcome(session, last) in {"completed", "bypassed"}:
        active_module = "none"
    return {
...
def format_brief_status(session: dict[str, Any], atoms: list[dict[str, Any]]) -> str:
    ...
    if position.get("status") == "completed" or (
        module == last and module_outcome(session, last) in {"completed", "bypassed"}
    ):
        return "Complete"
