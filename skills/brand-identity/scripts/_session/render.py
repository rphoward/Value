...
def fill_build_pack_file(session: dict[str, Any], template_name: str) -> str:
    """Parity stub only — BUILD_PACK_FILES is empty; do not add Values templates."""
    del session
    raise ValueError(
        f"brand-identity has no build-pack emitter (got {template_name!r}); "
        "BUILD_PACK_FILES is empty"
    )
