...
def resolve_milestone_path(session: Path, artifact_name: str) -> Path:
    """Resolve artifact under the session directory; reject path escape."""
    session_dir = session.parent.resolve()
    name = Path(artifact_name)
    if name.is_absolute() or ".." in name.parts or name.parts != (name.name,):
        raise ValueError(
            f"gate_artifacts value must be a plain filename under the session "
            f"directory, got {artifact_name!r}"
        )
    output_path = (session_dir / name.name).resolve()
    if not output_path.is_relative_to(session_dir):
        raise ValueError(
            f"Resolved milestone path escapes session directory: {output_path}"
        )
    return output_path
...
def main() -> int:
    ...
    try:
        output_path = resolve_milestone_path(args.session, artifact_name)
