...
def validation_milestone(session: dict[str, Any]) -> str:
    ...
    if all(outcomes[module] in {"completed", "bypassed"} for module in MODULE_ORDER):
        return "Lean Product Process Complete"
    if outcomes["ux-prototype"] in {"completed", "bypassed"}:
        return "Customer Test Ready"
    if outcomes["mvp-scope"] in {"completed", "bypassed"}:
        return "MVP Scoped"
    if outcomes["underserved-needs"] in {"completed", "bypassed"}:
        return "Needs Prioritized"
    if outcomes["customer-context"] in {"completed", "bypassed"}:
        return "Target Customer Defined"
    return "None"


def active_phase(session: dict[str, Any]) -> str:
    position = session["position"]
    if position.get("module") == "metrics" and module_outcome(session, "metrics") in {
        "completed",
        "bypassed",
    }:
        return "Complete"
    return MODULE_PHASE.get(position["module"], "Step 1: Target Customer")
...
def recompute_ledger(session: dict[str, Any]) -> dict[str, Any]:
    ...
    if module_outcome(session, "metrics") in {"completed", "bypassed"}:
        active_module = "none"
    return {
...
def format_brief_status(session: dict[str, Any], atoms: list[dict[str, Any]]) -> str:
    ...
    if position.get("status") == "completed" or (
        module == "metrics"
        and module_outcome(session, "metrics") in {"completed", "bypassed"}
    ):
        return "Complete"
