#!/usr/bin/env python3
"""One-shot normalizer: Google-Docs-exported teams suite -> prompt-suite contract."""
...
SOURCE = ROOT / "docs" / "High-Impact Tools Suite.md"
TARGET = SOURCE


def gdocs_unescape(text: str) -> str:
    """Undo common Google Docs markdown export escapes."""
    text = text.replace("\\!", "!")
...
def strip_image_refs(text: str) -> str:
    ...
    text = re.sub(
        r"\(sticky-note style,\s*words\)",
        "(sticky-note style, ≤10 words)",
        text,
    )
    text = re.sub(
        r"Always validate formulas \(\s*,\s*,\s*,\s*\)",
        "Always validate formulas (risk exposure, alignment index, psychological safety score, aligned autonomy)",
        text,
    )
    text = re.sub(
        r"Always validate formulas \(, , , \)",
        "Always validate formulas (risk exposure, alignment index, psychological safety score, aligned autonomy)",
        text,
    )
    text = re.sub(r"\n\[image\d+\]:.*?(?=\n\[image|\Z)", "", text, flags=re.DOTALL)
    return text.strip()


def extract_kb_json(raw: str) -> dict:
    start = raw.find("{")
    end = raw.rfind("}") + 1
    if start < 0 or end <= start:
        raise ValueError("KB JSON braces not found")
    blob = gdocs_unescape(raw[start:end])
    blob = blob.replace("\\\\", "\\")
    return json.loads(blob)


ORCHESTRATOR = """You are **High-Impact-Teams-Architect**, the Master AI Co-Pilot and Systems Architect trained on the Strategyzer framework *High-Impact Tools for Teams* by Stefano Mastrogiacomo and Alex Osterwalder.
...
TAM_PLANNER = """### SPECIALIZED PERSONA: TAM-Planner
...
TAM_ASSESSOR = """### SPECIALIZED PERSONA: TAM-Assessor
...
TEAM_CONTRACT = """### SPECIALIZED PERSONA: Team-Contract-Architect
...
PSYCH_SAFETY = """### SPECIALIZED PERSONA: Psych-Safety-Conflict-Resolver
...
def build_document(kb: dict) -> str:
...
def main() -> int:
    ...
    kb_start = raw.find("### **Section 1:")
    kb_end = raw.find("### **Section 2:")
    kb = extract_kb_json(raw[kb_start:kb_end])
    doc = build_document(kb)
    TARGET.write_text(doc, encoding="utf-8", newline="\n")
    print(f"Wrote normalized suite to {TARGET}")
    return 0
