"""Thin package contract for .cursor/skills/teams/ and skills/teams/."""
...
CURSOR_ROOT = ROOT / ".cursor" / "skills" / "teams"
SHIP_ROOT = ROOT / "skills" / "teams"
...
class TeamsSkillPackageTests(unittest.TestCase):
    def test_ship_tree_mirrors_cursor_tree(self) -> None:
        """Guards npx consumers getting a drifted teams package."""
        self.assertTrue(SHIP_ROOT.is_dir(), "skills/teams/ missing")
        self.assertEqual(mirror_mismatches(CURSOR_ROOT, SHIP_ROOT), [])

    def test_bounce_names_product_spine_slash(self) -> None:
        """Guards tam-planner done-enough losing the spine re-entry cue."""
        text = SKILL_MD.read_text(encoding="utf-8")
        self.assertIn("/product-spine", text)
        self.assertIn("after-tam-planner-gate", text)
    ...
    def test_workproduct_root_and_done_enough_milestone(self) -> None:
        ...
        text = SKILL_MD.read_text(encoding="utf-8")
        self.assertIn("workproduct/teams/", text)
        self.assertIn("tam-planner.md", text)
