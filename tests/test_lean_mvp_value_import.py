...
class LeanMvpValueImportTests(unittest.TestCase):
    ...
    def test_regression_import_does_not_overwrite_existing_lean_answer(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            slug = "acme-scheduler"
            value_dir = root / "workproduct" / "value-proposition" / slug
            ...
            lean_session = {
                ...
                "answers": [
                    {
                        "atom_id": "C01",
                        "answer": "Existing lean segment answer",
                        "kind": "fact",
                        "accepted_at": "2026-07-20T12:00:30Z",
                    }
                ],
                "evidence": [],
                "assumptions": [],
            ...
            self.assertEqual(imp.returncode, 0, imp.stderr)
            payload = json.loads(imp.stdout)
            self.assertNotIn("C01", payload["imported"])

            lean_session = json.loads(lean_path.read_text(encoding="utf-8"))
            c01_answers = [
                record for record in lean_session["answers"] if record["atom_id"] == "C01"
            ]
            self.assertEqual(len(c01_answers), 1)
            self.assertEqual(c01_answers[0]["answer"], "Existing lean segment answer")
            self.assertNotIn("provenance", c01_answers[0])
